...
class ProductionP13(Production):
    def extract_left_side(self) -> None | tuple:
        for node, data in self.graph.nodes(data=True):
            ...
            two_hanging_middles = self._find_two_hanging_middles(neighbors)
            if not two_hanging_middles:
                continue

            n1, m12, n2, n3, m34, n4 = two_hanging_middles
            if self._is_valid_middle(n1, m12, n2) and self._is_valid_middle(
                n3, m34, n4
            ):
                return (
                    self._extract_subgraph(node, neighbors + [m12, m34]),
                    two_hanging_middles,
                )
        return None
    ...
    def apply(self):
        ...
        if result is not None:
            (q, neighbors), (n1, m12, n2, n3, m34, n4) = result
            self._remove_nodes(q, neighbors, m12, m34)

            midpoints = {m12: (n1, n2), m34: (n3, n4)}
            self._create_midpoints(neighbors, midpoints)

            phantom_edge_data = {
                (n1, n2): self.graph.get_edge_data(n1, m12),
                (n3, n4): self.graph.get_edge_data(n3, m34),
            }
            self._fill_graph(neighbors, midpoints, edge_data=phantom_edge_data)
            self._update_subgraph_nodes(m12, m34)
    ...
    def _remove_nodes(self, q, neighbors, m12, m34):
        neighbors.remove(m12)
        neighbors.remove(m34)
